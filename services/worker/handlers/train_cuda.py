...
from services.worker.runtime.llamafactory import (
    LlamaFactoryRunner,
    SubprocessLlamaFactoryRunner,
    TrainerEvent,
    TrainerJobInput,
...
def run_train_cuda_job(
    session: Session,
    mib_home: Path,
    job_id: str,
    *,
    runner: LlamaFactoryRunner | None = None,
) -> str:
    store = TrainingStore(session)
    job = session.get(Job, job_id)
    if job is None:
        raise TrainCudaError("JOB_NOT_FOUND", "Job does not exist.", error_class="ARTIFACT_MISSING")
    model_run = _model_run_for_job(session, job)
    dataset = session.get(Dataset, model_run.dataset_id)
    if dataset is None:
        raise TrainCudaError("DATASET_NOT_FOUND", "Training dataset does not exist.", error_class="ARTIFACT_MISSING")
    _validate_cuda_job(job, model_run)
...
def _model_run_for_job(session: Session, job: Job) -> ModelRun:
    ...
    if model_run is None:
        raise TrainCudaError("MODEL_RUN_NOT_FOUND", "ModelRun does not exist for train job.", error_class="ARTIFACT_MISSING")
    return model_run


def _validate_cuda_job(job: Job, model_run: ModelRun) -> None:
    if job.type != "train":
        raise TrainCudaError("JOB_TYPE_UNSUPPORTED", "Job is not a train job.")
    if model_run.backend != "cuda" or model_run.method != "qlora":
        raise TrainCudaError(
            "TRAIN_BACKEND_UNSUPPORTED",
            "CUDA wrapper only handles backend=cuda method=qlora.",
...
def hyperparams(training_preset: str, hardware_profile_id: Any) -> dict[str, Any]:
    presets = {
        "quick": {"epochs": 1, "lora_rank": 4, "lora_alpha": 8, "learning_rate": 0.0005, "batch_size": 1, "grad_accumulation": 4},
        "balanced": {"epochs": 3, "lora_rank": 8, "lora_alpha": 16, "learning_rate": 0.0005, "batch_size": 1, "grad_accumulation": 8},
        "production": {"epochs": 5, "lora_rank": 16, "lora_alpha": 32, "learning_rate": 0.0003, "batch_size": 1, "grad_accumulation": 16},
    }
...
def _record_trainer_event(store: TrainingStore, job: Job, model_run: ModelRun, event: TrainerEvent) -> None:
    ts = utc_now()
    if event.kind == "log":
        store.append_event(
            job=job,
