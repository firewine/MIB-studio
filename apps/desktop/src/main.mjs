...
import { renderExportPage, renderPackagePage } from "./lib/workflowViews.mjs";
...
function page(route) {
  ...
  if (route.name === "projectPackage" || route.name === "projectExport") {
    const workflowViewDeps = {
      state,
      completedModelRun,
      validBenchmark,
      latestAgentPackage,
    ...
    return route.name === "projectPackage" ? renderPackagePage(workflowViewDeps) : renderExportPage(workflowViewDeps);
  }
